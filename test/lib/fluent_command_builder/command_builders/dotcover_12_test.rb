...
class DotCover11Test < CommandTestBase
  include FluentCommandBuilder::DotCover::V12
  ...
  def test_analyse_full_command
    ...
    actual = dotcover.analyse('configurationFile').analyse_target_arguments('bool').filters(%w(filter1 filter2)).inherit_console('bool').log_file('logFile').output('output').report_type('reportType').target_arguments('targetArguments').target_executable('targetExecutable').target_working_dir('targetWorkingDir').temp_dir('tempDir')
  ...
  def test_cover_full_command
    ...
    actual = dotcover.cover('configurationFile').analyse_target_arguments('bool').filters(%w(filter1 filter2)).inherit_console('bool').log_file('logFile').output('output').target_arguments('targetArguments').target_executable('targetExecutable').target_working_dir('targetWorkingDir').temp_dir('tempDir')
    assert_command expected, actual
  end
