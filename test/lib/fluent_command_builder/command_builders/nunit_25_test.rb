...
class NUnit25Test < CommandTestBase
  ...
  def test_full_command
    ...
    actual = nunit('inputFiles').fixture('fixture').load('fixture').run('test').config('config').xml('file').xml_console.output('file').out('file').err('file').labels.trace('level').include('category').exclude('category').framework('framework').process('process').domain('domain').no_shadow.no_thread.timeout('timeout').wait.no_logo.no_dots.help
