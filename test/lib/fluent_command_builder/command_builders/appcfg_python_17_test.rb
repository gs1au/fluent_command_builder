...
class AppCfgPython17Test < CommandTestBase
  include FluentCommandBuilder::AppCfgPython::V17
  ...
  def test_backends_update
    ...
    actual = appcfg_python.backends('directory').update('backend')
    assert_command expected, actual
  end
  ...
  def test_update
    ...
    actual = appcfg_python.update('directory').no_precompilation.backends
    assert_command expected, actual
  end
