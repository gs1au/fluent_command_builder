...
class Netsh61Test < CommandTestBase
  include FluentCommandBuilder::Netsh::V61
  ...
  def test_advfirewall_firewall_delete_rule_full_command
    ...
    actual = netsh.advfirewall.firewall.delete_rule('ruleName').dir('direction').profile('profile').program('program').service('serviceShortName').local_ip(%w(address1 address2)).remote_ip(%w(address1 address2)).local_port(%w(port1 port2)).remote_port(%w(port1 port2)).protocol('protocol')
    assert_command expected, actual
  end
  ...
  def test_advfirewall_firewall_set_rule_full_command_except_new
    ...
    actual = netsh.advfirewall.firewall.set_rule.group('groupName').dir('direction').profile('profile').program('program').service('serviceShortName').local_ip(%w(address1 address2)).remote_ip(%w(address1 address2)).local_port(%w(port1 port2)).remote_port(%w(port1 port2)).protocol('protocol')
    assert_command expected, actual
  end
  ...
  def test_advfirewall_firewall_show_rule_full_command
    ...
    actual = netsh.advfirewall.firewall.show_rule('ruleName').profile('profile').type('type').verbose
    assert_command expected, actual
  end
  ...
  def test_http_add_url_acl_full_command
    ...
    actual = netsh.http.add_url_acl('url').user('user').listen('listen').delegate('delegate').sddl('sddl')
    assert_command expected, actual
  end

  def test_http_delete_url_acl_full_command
    expected = 'netsh http delete urlacl url=url'
    actual = netsh.http.delete_url_acl('url')
    assert_command expected, actual
  end
