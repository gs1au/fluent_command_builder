...
class MSTest2010Test < CommandTestBase
  ...
  def test_full_command
    ...
    actual = mstest_2010.test_container('fileName').test_metadata('fileName').test_list('testListPath').category('testCategoryFilter').test('testName').no_isolation.test_settings('fileName').run_config('fileName').results_file('fileName').unique.detail('propertyId').help.no_logo.use_std_err.publish('serverName').publish_results_file('fileName').publish_build('buildId').team_project('teamProjectName').platform('platform').flavor('flavor')
