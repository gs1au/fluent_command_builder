...
class SevenZip92Test < CommandTestBase
  ...
  def test_delete_full_command
    ...
    actual = sevenzip_92.delete('archive', 'files').include('recurseType', 'fileRef').include('fileRef').set_compression_method('methodParameters').password('password').recursive('modifier').recursive.update_options('args').working_dir('dirPath').working_dir.exclude('recurseType', 'fileRef').exclude('fileRef')
    assert_command expected, actual
  end
  ...
  def test_extract_full_command
    ...
    actual = sevenzip_92.extract('archive', 'files').ai.an.ao.include('fileRef', 'recurseType').include('fileRef').output('dirPath').password('password').recursive('modifier').recursive.write_data_to_stdout.type('archiveType').exclude('fileRef', 'recurseType').exclude('fileRef').y
    assert_command expected, actual
  end
  ...
  def test_list_full_command
    ...
    actual = sevenzip_92.list('archive').ai.an.ax.include('recurseType', 'fileRef').include('fileRef').slt.password('password').recursive('modifier').recursive.type('archiveType').exclude('fileRef', 'recurseType').exclude('fileRef')
    assert_command expected, actual
  end
  ...
  def test_test_full_command
    ...
    actual = sevenzip_92.test('archive', 'files').ai.an.ax.include('fileRef', 'recurseType').include('fileRef').password('password').recursive('modifier').recursive.exclude('fileRef', 'recurseType')
    assert_command expected, actual
  end
  ...
  def test_extract_with_full_paths_full_command
    ...
    actual = sevenzip_92.extract_with_full_paths('archive', 'files').ai.an.ao.include('fileRef', 'recurseType').include('fileRef').output('dirPath').password('password').recursive('modifier').recursive.write_data_to_stdout.type('archiveType').exclude('fileRef', 'recurseType').exclude('fileRef').y
    assert_command expected, actual
  end
