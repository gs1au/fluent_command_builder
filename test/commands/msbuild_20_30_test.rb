...
class MSBuild20And35Test < CommandTestBase
  ...
  def test_full_command
    ...
    assert_each_command expected, msbuild_20('projectFile'), msbuild_30('projectFile') do |msbuild|
      msbuild.help.no_logo.version.file('file').no_auto_response.target(['target1', 'target2']).property({'key1' => 'value1', 'key2' => 'value2'}).logger('logger').console_logger_parameters('parameters').verbosity('level').no_console_logger.validate('schema').validate
    end
  end
